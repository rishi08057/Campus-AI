...
logger = logging.getLogger("backend.health_vector_service")

class HealthVectorService:
    def __init__(self):
            ...
            try:
                ...
                if "429" in error_str or "resource_exhausted" in error_str or "quota exceeded" in error_str:
                    if attempt < max_retries - 1:
                        logger.warning(f"Rate limited (429) on embedding. Retrying in {retry_delay}s... (Attempt {attempt + 1}/{max_retries})")
    ...
    def index_documents(self, documents: List[Dict[str, Any]]):
        """Index health documents into the vector database."""
        if not documents:
            return
        ...
        logger.info(f"Indexed {len(documents)} health documents into ChromaDB.")

    def search_documents(self, query: str, n_results: int = 3) -> List[Dict[str, Any]]:
        """Find the most relevant health documents for a query."""
        try:
            ...
            logger.error(f"Health search error: {e}")
            return []
