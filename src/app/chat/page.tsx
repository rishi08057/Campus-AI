...
import { getAllSessions, ChatSession, deleteChatSession } from "@/lib/chatStorage";
...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    // Start with a new session if none is selected
    if (!sessionId) {
      setSessionId(uuidv4());
    }
  }, [sessionId]);
      ...
      <div className="flex flex-col md:flex-row gap-6 h-[80vh]">
        ...
        <aside className="w-full md:w-64 flex flex-col gap-4 bg-white/50 backdrop-blur rounded-[2rem] border border-slate-200/80 p-4 shadow-sm">
          ...
          <div className="flex-1 overflow-y-auto space-y-2 pr-1">
            ...
            {sessions.length === 0 ? (
              <p className="text-xs text-slate-400 px-2 italic">No history yet</p>
            ) : (
              sessions.map((session) => (
                ...
                >
                  <div className="flex items-center gap-2 min-w-0">
                    <svg className={`w-4 h-4 flex-shrink-0 ${sessionId === session.id ? 'text-sky-500' : 'text-slate-400'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
